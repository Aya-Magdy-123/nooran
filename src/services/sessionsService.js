...
export async function addSession(form, teacherName) {
  ...
  const sessionShift = _getSessionShift({ trialTime: form.trialTime, regularDates: form.regularDates })
  ...
  if (sessionShift) {
    ...
    if (availableSups.length) {
      ...
      const assigned = availableSups.reduce((min, s) =>
        counts[s.id] < counts[min.id] ? s : min
      , availableSups[0])
